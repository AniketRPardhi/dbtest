...
traffic_df = (df.groupBy("traffic_source")
              .agg(round(sum("revenue"), 10).alias("total_rev"),
                   round(avg("revenue"), 10).alias("avg_rev"))
             )
...
# TODO
bonus_df = (top_traffic_df.withColumn("avg_rev", round(col("avg_rev"),2)).withColumn("total_rev", round(col("total_rev"),2))
)
...
chain_df = (df.groupBy("traffic_source")
            .agg(round(sum("revenue"), 10).alias("total_rev"),round(avg("revenue"), 10).alias("avg_rev"))
            .orderBy(desc("total_rev")).limit(3)
