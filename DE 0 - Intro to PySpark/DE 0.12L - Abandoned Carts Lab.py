...
converted_users_df = (sales_df.select("email").distinct()
...
abandoned_items_df = (abandoned_carts_df.select(explode(col("cart")).alias('items'))
                     .groupBy(col("items.product")).count()
